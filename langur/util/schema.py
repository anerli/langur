...
# Requires testing, a lot of this is Claude-generated
...
def schema_from_function(fn: Callable) -> SchemaResult:
    """
    Convert a function's signature to a Pydantic schema with Fields preserving descriptions.
    Ignores ActionContext parameters for JSON schema compatibility.
    ...
    # Copy signature but exclude ActionContext parameters
    new_params = []
    for param_name, param in signature.parameters.items():
    ...
    # Create fields_dict with Field objects, including ALL original parameters
    fields_dict = {}
    for param_name, param in signature.parameters.items():
        field_kwargs = {}
